...
import { createDefaultSfx } from '@/types'
import type { SfxItem } from '@/types'

const libs = useLibrariesStore()

const editingItem = ref<SfxItem | null>(null)
const pendingFile = ref<File | null>(null)

function openNew() {
  editingItem.value = createDefaultSfx()
  pendingFile.value = null
}

function edit(item: SfxItem) {
...
function save() {
  ...
  libs.saveSfx(editingItem.value, pendingFile.value ?? undefined)
...
<template>
  <div class="space-y-4">
      ...
      <div v-if="editingItem" class="card space-y-4 border-l-4 border-l-primary/50">
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="editingItem.name" class="input" placeholder="如：电话铃声" />
          </div>
          <div>
            <label class="label">音频文件</label>
            <input ref="fileInput" type="file" accept="audio/*" @change="onFileChange" class="input text-sm file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" />
          </div>
        </div>
        <div>
          <label class="label">描述（AI 分析用）</label>
          <input v-model="editingItem.description" class="input" placeholder="简要描述音效效果" />
        </div>
    ...
    <!-- 列表 -->
    <div v-if="libs.sfxLibrary.length === 0 && !editingItem" class="text-center py-8 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
      暂无音效，点击右上角添加
    </div>
    
    <div v-else class="space-y-2">
      <div v-for="item in libs.sfxLibrary" :key="item.id"
      ...
      >
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 text-xs">SFX</div>
          <div>
        ...
        <div class="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          ...
          <button @click="libs.deleteSfx(item.id)" class="btn btn-ghost p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg">✕</button>
        </div>
      </div>
    </div>
  </div>

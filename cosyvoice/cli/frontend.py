...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        self.speech_tokenizer_session = onnxruntime.InferenceSession(speech_tokenizer_model, sess_options=option,
                                                                     providers=["CPUExecutionProvider"])  # 使用 CPU 以释放 GPU 显存给 vLLM
        ...
        if self.use_ttsfrd:
            ...
            # wetext model path - check both possible structures
            wetext_dir = os.path.abspath('pretrained_models/wetext')
            # 第一种结构：直接在wetext下 (zh/tn/)
            # 第二种结构：在pengzhendong/wetext下 (pengzhendong/wetext/zh/tn/)
            
            zh_tagger_path = os.path.join(wetext_dir, 'zh', 'tn', 'tagger.fst')
            en_tagger_path = os.path.join(wetext_dir, 'en', 'tn', 'tagger.fst')
            
            # 检查pengzhendong结构
            if not os.path.exists(zh_tagger_path):
                alt_wetext_dir = os.path.join(wetext_dir, 'pengzhendong', 'wetext')
                alt_zh_tagger = os.path.join(alt_wetext_dir, 'zh', 'tn', 'tagger.fst')
                if os.path.exists(alt_zh_tagger):
                    wetext_dir = alt_wetext_dir
                    zh_tagger_path = alt_zh_tagger
                    en_tagger_path = os.path.join(wetext_dir, 'en', 'tn', 'tagger.fst')
            
            if os.path.exists(zh_tagger_path) and os.path.exists(en_tagger_path):
                print(f"✅ Loading wetext model from local: {wetext_dir}")
                self.zh_tn_model = ZhNormalizer(
                    tagger_path=zh_tagger_path,
                    verbalizer_path=os.path.join(wetext_dir, 'zh', 'tn', 'verbalizer.fst'),
                    remove_erhua=False,
                    lang='zh'
                )
                self.en_tn_model = EnNormalizer(
                    tagger_path=en_tagger_path,
                    verbalizer_path=os.path.join(wetext_dir, 'en', 'tn', 'verbalizer.fst'),
                    lang='en'
                )
            else:
                # Fallback to default wetext (will download or use cache)
                print(f"⚠️ Local wetext model not found, downloading from modelscope cache...")
